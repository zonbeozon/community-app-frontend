...
import type { Errors, FormHandler, Validator } from '@/types/form.type';

export const useForm = <T extends object | string>(initialState: T, validate: Validator<T>) => {
  ...
  const setValuesDirectly = useCallback(
    (newValues: T) => {
      setValues(newValues);
      setErrors(validate(newValues));
    },
