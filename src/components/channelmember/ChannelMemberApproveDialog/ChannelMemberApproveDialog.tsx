...
const ChannelMemberApproveDialog = ({ open, onOpenChange, channelId, targetMember }: MemberDialogProps) => {
  const { mutate: approveMember, isPending } = useApproveChannelMember();

  const handleApprove = () => {
    approveMember({ channelId, targetMemberId: targetMember.memberId }, {
      onSuccess: () => {
        onOpenChange(false);
      },
    });
