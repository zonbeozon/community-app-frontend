...
export const ChannelProfileImage = React.forwardRef<HTMLDivElement, ChannelProfileImageProps>(
  ({ channelInfo, size, className, ...props }, ref) => {
    ...
    const imageElement = channelInfo.profile?.imageUrl ? (
      <img src={channelInfo.profile.imageUrl} alt={`${channelInfo.title} 채널의 프로필`} className={imageClass} />
    ) : (
      <div className={fallbackClass}>{firstLetter}</div>
    );

    return (
      <div ref={ref} {...props} className={`${wrapperClass || ''} ${className || ''}`.trim()}>
        {imageElement}
      </div>
    );
  },
