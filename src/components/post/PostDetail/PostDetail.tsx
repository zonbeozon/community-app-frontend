...
const PostDetail = () => {
  ...
  const { data: postData, isLoading: isLoadingPost, isError: isErrorPost } = useGetPost(numericPostId);
  ...
  if (isLoadingPost) {
    return <div className={S.wrapper}><ItemSkeleton /></div>;
  }

  if (isErrorPost || !postData?.post) {
  ...
  return (
    <div className={S.wrapper}>
      ...
      <div className={S.inputContainer}>
        <CommentInput onSubmit={handleCommentSubmit} isSubmitting={isCreatingComment} />
      </div>
    </div>
  );
};
